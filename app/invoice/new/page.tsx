...
export default function NewInvoicePage() {
  ...
  const inp = { width: '100%', padding: '10px 14px', borderRadius: '8px', border: `1px solid ${BORDER}`, background: 'rgba(255,255,255,0.05)', color: TEXT, fontSize: '14px', outline: 'none', boxSizing: 'border-box' as const };
  const lbl = { display: 'block' as const, fontSize: '11px', fontWeight: '700' as const, color: MUTED, marginBottom: '6px', letterSpacing: '0.5px', textTransform: 'uppercase' as const };
    ...
    <div style={{ minHeight: '100vh', background: '#0A0A0F', color: TEXT, fontFamily: 'system-ui, sans-serif' }}>
      ...
      <main style={{ maxWidth: '720px', margin: '0 auto', padding: '40px' }}>
        ...
        <div style={{ background: CARD, borderRadius: '14px', border: `1px solid ${BORDER}`, padding: '24px', marginBottom: '14px' }}>
          <h2 style={{ fontSize: '14px', fontWeight: '700', marginBottom: '18px' }}>From</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            <div><label style={lbl}>Your name *</label><input style={inp} value={senderName} onChange={e => setSenderName(e.target.value)} placeholder="Jane Smith" /></div>
            <div><label style={lbl}>Your email *</label><input style={inp} type="email" value={senderEmail} onChange={e => setSenderEmail(e.target.value)} placeholder="you@example.com" /></div>
            <div style={{ gridColumn: '1 / -1' }}><label style={lbl}>Your Starknet wallet address *</label><input style={inp} value={senderWallet} onChange={e => setSenderWallet(e.target.value)} placeholder="0x..." /></div>
          </div>
        </div>

        {/* To */}
        <div style={{ background: CARD, borderRadius: '14px', border: `1px solid ${BORDER}`, padding: '24px', marginBottom: '14px' }}>
          <h2 style={{ fontSize: '14px', fontWeight: '700', marginBottom: '18px' }}>To</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            <div><label style={lbl}>Client name *</label><input style={inp} value={clientName} onChange={e => setClientName(e.target.value)} placeholder="Acme Corp" /></div>
            <div><label style={lbl}>Client email *</label><input style={inp} type="email" value={clientEmail} onChange={e => setClientEmail(e.target.value)} placeholder="client@example.com" /></div>
          </div>
        </div>

        {/* Details */}
        <div style={{ background: CARD, borderRadius: '14px', border: `1px solid ${BORDER}`, padding: '24px', marginBottom: '14px' }}>
          <h2 style={{ fontSize: '14px', fontWeight: '700', marginBottom: '18px' }}>Details</h2>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            {items.map((item, idx) => (
              <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 110px 36px', gap: '8px', marginBottom: '8px', alignItems: 'center' }}>
                ...
                <button onClick={() => removeItem(idx)} disabled={items.length === 1} style={{ background: 'none', border: 'none', cursor: items.length === 1 ? 'not-allowed' : 'pointer', color: MUTED, fontSize: '20px', opacity: items.length === 1 ? 0.3 : 1 }}>×</button>
              </div>
            ))}
            <button onClick={addItem} style={{ marginTop: '6px', fontSize: '13px', color: CORAL, background: 'none', border: 'none', cursor: 'pointer', fontWeight: '600', padding: 0 }}>+ Add line item</button>
          </div>

          {/* Total */}
          <div style={{ borderTop: `1px solid ${BORDER}`, paddingTop: '16px', display: 'flex', justifyContent: 'flex-end' }}>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '11px', color: MUTED, fontWeight: '700', letterSpacing: '0.5px', marginBottom: '4px' }}>TOTAL</div>
          ...
          <div style={{ marginTop: '16px' }}>
            ...
            <textarea style={{ ...inp, height: '80px', resize: 'vertical' as const }} value={notes} onChange={e => setNotes(e.target.value)} placeholder="Payment terms or additional info..." />
          </div>
        </div>
